        ...
        # Ensure valid # of shares
            ...
            if cash - total_cost < 0:
                ...
                if symbol['symbol'] in symbols:
                    ...
                    db.execute("UPDATE portfolio (shares, total) VALUES (?, ?) WHERE symbol = ? AND user_portfolio_id = ?", int(current_shares)+int(shares), int(current_total)+int(total_cost), symbol['symbol'], session["user_id"])
